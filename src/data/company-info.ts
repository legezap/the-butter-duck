export const COMPANY = {
  name: "The Butter Duck Company FZC LLC",
  license: "4426620.01",
  country: "United Arab Emirates",
} as const;

export const OFFICE = {
  line1: "Marina Plaza, 29th Floor",
  line2: "Dubai Marina, Dubai, UAE",
  short: "Marina Plaza, 29th Floor, Dubai, UAE",
} as const;

export const CONTACT = {
  phone: "+971 52 147 7966",
  phoneTel: "tel:+971521477966",
  email: "hello@thebutterduck.com",
  emailOperations: "operations@thebutterduck.com",
  whatsapp: "https://wa.me/971521477966?text=Hi%2C%20I%27m%20interested%20in%20an%20exhibition%20stand.%20Can%20we%20discuss%3F",
  instagram: "https://instagram.com/thebutterduck",
  instagramHandle: "@thebutterduck",
  linkedin: "https://linkedin.com/company/thebutterduck",
} as const;
